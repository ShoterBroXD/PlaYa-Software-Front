...
export class CommunitiesIndexComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    setTimeout(() => {
      if (this.isLoading) {
        console.warn('TIMEOUT: Forzando fin de carga después de 10s');
        this.subscription?.unsubscribe();
        this.isLoading = false;
        this.error = 'Tiempo de espera agotado. Verifica que el backend esté ejecutándose.';
  ...
  loadCommunities() {
    ...
    console.log('Cargando comunidades desde el backend...');
    this.subscription = this.communityService.getAllCommunities().subscribe({
      next: (communities: CommunityResponseDto[]) => {
        console.log('Respuesta del backend recibida:', communities.length, 'comunidades');
        ...
        if (communities.length > 0) {
          ...
          this.activeCommunities = mapped.slice(0, 8);
          this.topCommunities = mapped.slice(8, 16);
          this.isLoading = false;
          console.log(`${communities.length} comunidades cargadas exitosamente`);
          console.log('activeCommunities:', this.activeCommunities.length);
          console.log('isLoading:', this.isLoading);
        } else {
          console.log('Base de datos vacía - No hay comunidades');
      ...
      error: (error) => {
        ...
        this.error = 'Error al cargar las comunidades. Verifica que el backend esté ejecutándose.';
        this.activeCommunities = [];
        this.topCommunities = [];
        this.isLoading = false;
        this.cdr.detectChanges();
