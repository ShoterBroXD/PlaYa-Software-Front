...
export class PlayerCommentsSidebarComponent {
  ...
  private fetchComments(songId: number) {
    ...
    this.commentService.getCommentsBySong(songId).subscribe({
      next: (response) => {
  ...
  private buildTree(dtos: CommentResponse[]): Comment[] {
    ...
    dtos.forEach((dto) => {
      byId.set(dto.idComment, {
        id: dto.idComment,
        userId: dto.idUser,
        username: `Usuario ${dto.idUser}`,
        avatar: '/assets/img/icons/user.png',
        content: dto.content,
        timestamp: dto.date ? new Date(dto.date) : new Date(),
        likes: 0,
        isLiked: false,
        parentId: dto.parentComment ?? null,
        replies: [],
      });
    });
  ...
  postComment() {
    ...
    this.commentService
      .createComment({ idSong: this.currentSongId, content })
      .subscribe({
        next: () => {
          this.newComment = '';
  ...
  submitReply(commentId: number) {
    ...
    this.commentService
      .createComment({ idSong: this.currentSongId, content, parentComment: commentId })
      .subscribe({
        next: () => {
